# Аренда PRO: React + Django + SQLite

Готовый учебный сервис для веб-приложения учёта аренды коммерческой недвижимости. Фронт перенесён из исходного `App.js` и больше не хранит основную БД в `data.js`: при запуске он загружает данные из Django API `/api/data/`, а операции создания, изменения и удаления отправляет на сервер.
...
В seed-файле и SQLite-БД находятся:
...
## Спроектированная реляционная БД
